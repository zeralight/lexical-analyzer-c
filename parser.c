...
AST* Constant(TokenList** tokens) {
  ...
  if (ProcessToken(tokens, TOKEN_INTEGER)) {
    AST* ast = MakeAST(NODETYPE_CONSTANT_INTEGER, curr_token->filename, curr_token->linenum);
...
*/
AST* BinaryExprLogicalOr(TokenList** tokens) {
  fprintf(stderr, "BinaryExprLogicalOr\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprLogicalAnd(TokenList** tokens) {
  fprintf(stderr, "BinaryExprLogicalAnd\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprBitwiseOr(TokenList** tokens) {
  fprintf(stderr, "BinaryExprBitwiseOr\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprBitwiseXor(TokenList** tokens) {
  fprintf(stderr, "BinaryExprBitwiseXor\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprBitwiseAnd(TokenList** tokens) {
  fprintf(stderr, "BinaryExprBitwiseAnd\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprEquality(TokenList** tokens) {
  fprintf(stderr, "BinaryExprEquality\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprComparison(TokenList** tokens) {
  fprintf(stderr, "BinaryExprComparison\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprAddSub(TokenList** tokens) {
  fprintf(stderr, "BinaryExprAddSub\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
*/
AST* BinaryExprMultDiv(TokenList** tokens) {
  fprintf(stderr, "BinaryExprMultiDiv\n");
  if (*tokens == NULL) {
    /* Error */
    generate_eof_error();
    return MakeAST(NODETYPE_ERR, curr_filename, -1);
...
AST* BinaryExpr(TokenList** tokens,
  ...
  TokenList *token = *tokens;
  while (token && count_parentheses >= 0) {
    if (token->t->type == TOKEN_SYM_OPENPREN) ++count_parentheses;
    else if (token->t->type == TOKEN_SYM_CLOSEPREN) --count_parentheses;
    token = token->next;
  }
  if (count_parentheses != 0) {
    AST* ast = MakeAST(NODETYPE_ERR, (*tokens)->t->filename, (*tokens)->t->linenum);
    return ast;
  }
