...
/*
  ...
  reused).
*/
size_t SelectToken(char *buffer, size_t size, size_t *linenum, TokenList **tokens, char *filename)
{
  ...
  */
  ...
  { // / and comments
    if (size_read + 1 == size) return size_read;
    if (buffer[size_read + 1] == '/')
    {
      size_t size_read_old = size_read;
  ...
  { // #include /s* "string" or error
    if (strlen(PREPROCESSOR_PRAGMA) + size_read + 1 >= size)
    {
      return size_read;
    }
    int match = strncmp(buffer + size_read, PREPROCESSOR_PRAGMA, strlen(PREPROCESSOR_PRAGMA)) == 0
                && is_space(buffer[size_read + strlen(PREPROCESSOR_PRAGMA)]) && buffer[size_read + strlen(PREPROCESSOR_PRAGMA)] != '\n'; // must not accept '#include123'
    size_t str_len = 0;
    if (match)
    {
    ...
    {
      int total = generate_generic_error(&t, buffer, size_read, size, *linenum, filename);
      if (total == 0) return size_read;
      else size_read += total;
    }
  }
  else if (buffer[size_read] == '\'')
  { // characters and some errors
    if (size_read + 1 >= size) return size_read;
    size_t j = size_read + 1;
    for (; j < size && j < size_read + 4; ++j)
    {
      if (j+1 < size && replace_escape_in_character(buffer + j) != -1) ++j;
      else if (buffer[j] == '\n') ++*linenum;
      else if (buffer[j] == '\'') break;
    }
    if (j < size && isprint(buffer[size_read + 1]) 
    && (j == size_read + 2 || (j == size_read + 3 && replace_escape_in_character(buffer + size_read + 1) != -1)))
    {
      t = create_token(filename);
      t->linenum = *linenum;
      t->type = TOKEN_CHARACTER;
      t->data.character = j == size_read + 2 ? buffer[size_read+1] : replace_escape_in_character(buffer + size_read + 1);
      size_read = j+1;
    }
    else
    {
  ...
  { // strings and some errors
    ...
    while (size_read + str_len < size && search)
    {
      if (buffer[size_read + str_len] == '\\')
      {
        if (size_read + str_len + 1 < size && replace_escape_in_string(buffer + size_read + str_len) != -1)
        {
          str_len += 2;
        }
        else
  ...
  else if (is_digit(buffer[size_read]))
  { // positive integers and some errors
    size_t int_len = 1;
    int search = 1;
    ...
    else
    {
      ...
      t->data.integer = strtol(buffer + size_read, NULL, 10);
      size_read += int_len;
    }
    
  }
  else
  { // Identifiers, keywords, and errors
    size_t id_len = 1;
    int search = 1;
    ...
    else
    {
      ...
      else if (is_valid_identifier(token_contents))
      {
        ...
        t->data.identifier = malloc(id_len+1);
        strcpy(t->data.identifier, token_contents);
        size_read += id_len;
      }
      else
      {
